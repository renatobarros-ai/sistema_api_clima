"""
Módulo do exportador de dados para formato Parquet.
"""
...
class ExportadorParquet(BaseExportador):
    """
    Exportador de dados climáticos para arquivos Parquet.
    
    Formato colunar otimizado para análise de dados, adequado para
    ...
    def exportar(
        ...
        """
        Exporta os dados para formato Parquet.
        ...
        if df.empty:
            logger.warning(f"DataFrame vazio para {localidade}")
            return []
        
        # Define o nome do arquivo
        nome_arquivo = self._gerar_nome_arquivo(localidade, tipo_dados, "parquet")
        
        # Salva o DataFrame como Parquet
        try:
            # Tenta converter todas as colunas para tipos compatíveis com Parquet
            for coluna in df.columns:
                # Converte colunas de objeto para string
                if df[coluna].dtype == 'object':
                    df[coluna] = df[coluna].astype(str)
            
            df.to_parquet(nome_arquivo, index=False)
            logger.info(f"Dados exportados para Parquet: {nome_arquivo}")
            return [nome_arquivo]
        
        except Exception as e:
            logger.error(f"Erro ao exportar para Parquet: {str(e)}")
            return []
    ...
    def exportar_multiplas_localidades(
        ...
        # Modo concatenado: um único arquivo com todos os dados
            ...
            nome_arquivo = os.path.join(
                ...
                f"clima_todas_localidades_{tipo_dados}.parquet"
            )
            ...
            try:
                ...
                for coluna in df_concatenado.columns:
                    # Converte colunas de objeto para string
                    if df_concatenado[coluna].dtype == 'object':
                        df_concatenado[coluna] = df_concatenado[coluna].astype(str)
                
                df_concatenado.to_parquet(nome_arquivo, index=False)
                logger.info(f"Dados concatenados exportados para Parquet: {nome_arquivo}")
                arquivos_gerados.append(nome_arquivo)
            
            except Exception as e:
                logger.error(f"Erro ao exportar dados concatenados para Parquet: {str(e)}")
